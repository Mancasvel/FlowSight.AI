...
*   **Hybrid Context Engine**:
    *   **Generative Vision**: Uses **Qwen 3 VL** to generate detailed human-readable descriptions of screen activity (e.g., "Editing a Rust struct").
    *   **Contextual Intelligence**: Uses **Qwen 3 (1.7B)** on the PM Dashboard to aggregate and summarize team activity every 5 minutes.
...
| **Generative AI** | **Ollama** | Runs local Qwen models (Qwen 3 VL, Qwen 3) for text generation. |
...
2.  **Contextualize**:
    ...
    *   **Ollama (Qwen 3 VL)** analyzes the screenshot to generate a text summary.
...
6.  **Summarize**: A background job runs every 5 mins, using **Qwen 3** to generate high-level context summaries.

```mermaid
graph TD
    subgraph Developer Machine [Agent]
        Screen[Screen Buffer] -->|Image| Ollama[Ollama (Qwen 3 VL)]
    ...
    subgraph Manager Machine [PM Dashboard]
        ...
        DB -->|Query| Summarizer[Background Job (Qwen 3)]
...
*   **Ollama**: Installed and running.
    *   **Models Required**: `qwen3-vl:2b` (Agent), `qwen3:1.7b` (PM).
...
*   `vision_model`: AI Model selection (Default: `qwen3-vl:2b`).
